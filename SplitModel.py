...
if __name__ == '__main__':
    ...
    front = FrontEndNet(latent_size=320)
    ...
    torch.onnx.export(front, dummy_input, "MobileFaceNet_FrontShallow.onnx", input_names=["input"], output_names=["output"], do_constant_folding=True, export_params=True, opset_version=14)

    dummy_input = torch.randn(1, 320, 7, 7, device=device)
    torch.onnx.export(back, dummy_input, "MobileFaceNet_BackShallow.onnx", input_names=["input"], output_names=["output"], do_constant_folding=True, export_params=True, opset_version=14)